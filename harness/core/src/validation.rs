    ...
    for slice in &queue.slices {
        ...
        if slice.human_check_needed.required && slice.human_check_needed.resolved_at.is_none() {
            issues.push(issue(
                ...
                "human_check_pending",
                format!(
                    "slice `{}` is gated on an unresolved human check; prepare \
                     and finalize will refuse until `update-slice --resolve-human-check` runs \
...
pub fn validate_slice_contract(slice: &Slice) -> Result<()> {
    ...
    slice
        .state_target()
        .with_context(|| format!("slice `{}` has invalid `context_to_update`", slice.id))?;
