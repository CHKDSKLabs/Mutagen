...
use crate::structural::required_output_contract_for_author;
...
pub fn prepare_dispatch(options: PrepareDispatchOptions) -> Result<PrepareDispatchResult> {
    ...
    verify_active_queue_readiness(&queue_path, &active_state)?;
...
fn verify_active_queue_readiness(
    ...
    let Some(expected) = active_state.queue_readiness.as_ref() else {
        bail!(
            "active slice `{}` has no queue validation snapshot; re-prepare the slice through the harness before dispatch",
            active_state.slice_id
        );
    };
    ...
    {
        bail!(
            "queue validation changed after slice `{}` was activated; re-run /mutagen:slice before dispatch",
            active_state.slice_id
        );
    }
    ...
    {
        bail!(
            "queue validation basis changed after slice `{}` was activated; re-run /mutagen:slice before dispatch",
            active_state.slice_id
        );
    }
...
fn render_author_prompt(
    ...
    slice: &Slice,
    active_state: &crate::state::ActiveSliceState,
    state_target: &crate::state_target::StateTarget,
    scope_enforcement: ScopeEnforcementMode,
...
fn push_output_contract(lines: &mut Vec<String>, author_agent: &str) {
    let Some(sections) = required_output_contract_for_author(author_agent) else {
        return;
    };

    lines.push("### Output contract".to_string());
    lines.push(format!(
        "- The {author_agent} structural check requires every marker below as a markdown heading. Bold text is not a heading. Missing any marker fails the slice."
    ));
    for section in sections {
        lines.push(format!("- `{}`", section.marker));
    }
    lines.push("### Output skeleton".to_string());
    lines.push("````markdown".to_string());
    for section in sections {
        if section.marker == "State Update" {
            lines.push(format!("## {}", section.marker));
            lines.push(String::new());
            lines.push("```".to_string());
            lines.push("### <slice-id> — <YYYY-MM-DD>".to_string());
            lines.push(String::new());
            lines.push("<state update notes>".to_string());
            lines.push("```".to_string());
            continue;
        }
