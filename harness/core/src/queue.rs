...
pub enum SliceStatus {
    ...
    Escalated,
    Refused,
    // L1-Harness-002: finalize verifies write_set artifacts landed on disk.
    // Per the 2026-05-05 L1-Infra-003 postmortem, a slice can clear Tiger Claw
    // and still have authored zero real files. This bucket parks the slice
    // until the operator either re-runs author or invokes the
    // --accept-missing-artifacts override.
    FinalizationFailed,
}
...
impl HumanCheckNeeded {
    pub fn is_unresolved(&self) -> bool {
        self.required && self.resolved_at.is_none()
    }
}
...
pub struct BlockedSlice {
    ...
    pub pending_human_check: Option<String>,
}
...
impl SliceQueue {
    pub fn select_next_ready_slice(&self) -> NextSliceSelection {
        ...
        for (index, slice) in self.slices.iter().enumerate() {
            ...
            let pending_human_check = slice.pending_human_check_reason();
            if unmet_dependencies.is_empty() && pending_human_check.is_none() {
...
impl Slice {
    pub fn state_target(&self) -> Result<StateTarget> {
        StateTarget::parse(&self.context_to_update)
    }

    pub fn has_pending_human_check(&self) -> bool {
        self.human_check_needed.is_unresolved()
    }

    pub fn pending_human_check_reason(&self) -> Option<String> {
        self.has_pending_human_check()
            .then(|| self.human_check_needed.reason.trim().to_string())
            .map(|reason| {
                if reason.is_empty() {
                    "human check is unresolved".to_string()
                } else {
                    reason
                }
            })
