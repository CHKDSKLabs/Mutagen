...
pub fn record_review_verdict(
    ...
    let result = {
        ...
        // Bishop is advisory-only by design — see POSTMORTEM-2026-05-10 action
        // item #5. A `reviews/<slice-id>/bishop.md` may exist on disk (and is
        // useful context for a human reading the slice afterwards), but the
        // harness never opens it and never gates on its verdict. If Bishop ever
        // needs to gate, this is the site that needs a real parser + the
        // BishopVerdict variants (Clean / Advisory / Block) wired through
        // review_decision. Until then: Skip means "we didn't ask".
...
// The closed set of accepted verdict words: clean / gap / defect / skip / refused.
// (We also recognize the legacy compound token `qa-defect`, and the human phrasing
// `no adversarial surface` which maps to skip.) Anything else is silently ignored —
// upstream callers wrap a None return as a parse failure.
pub fn parse_tiger_claw_verdict(report: &str) -> Option<TigerClawVerdict> {
    ...
    for line in report.lines() {
        ...
        if !in_verdict_section {
            if is_verdict_heading(trimmed) {
                in_verdict_section = true;
                continue;
            }

            if let Some(rest) = strip_verdict_label(trimmed)
                && let Some(verdict) = classify_verdict_line(rest)
            {
                return Some(verdict);
            }
            continue;
...
// A heading anchors the verdict section if the hashes are followed by exactly the
// single word "verdict" (case-insensitive). `## Verdict`, `### Verdict`, `#### Verdict`
// all qualify; `## Verdict Notes` or `## Bishop Verdict` do not — extra tokens after
// the hashes mean it's a different section that just happens to mention the word.
fn is_verdict_heading(line: &str) -> bool {
    ...
    first.eq_ignore_ascii_case("verdict")
}
...
fn classify_verdict_line(line: &str) -> Option<TigerClawVerdict> {
    ...
    if normalized.contains("skip")
        ...
        || normalized.contains("no adversarial surface")
    {
