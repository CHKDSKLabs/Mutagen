...
pub fn materialize_cohort_worktrees(
    ...
    let (pruned_orphans, prune_warnings) = prune_orphan_worktree_roots(&workspace_root);
    ...
    Ok(MaterializeCohortWorktreesResult {
        ...
        members,
        pruned_orphans,
        cleanup_warnings: prune_warnings,
    })
}

// Sweep .mutagen/worktrees/ for stale roots that git no longer tracks.
// An "orphan" root is any direct child dir under .mutagen/worktrees/
// whose nested worktrees are absent from `git worktree list`. Returns
// (removed_paths, warnings_for_paths_we_could_not_remove).
fn prune_orphan_worktree_roots(workspace_root: &Path) -> (Vec<String>, Vec<String>) {
    let worktrees_root = workspace_root.join(".mutagen/worktrees");
    if !worktrees_root.is_dir() {
        return (Vec::new(), Vec::new());
    }

    let tracked = match all_managed_worktrees(workspace_root) {
        Ok(paths) => paths,
        Err(err) => {
            return (
                Vec::new(),
                vec![format!("could not list git worktrees during prune: {err}")],
            );
        }
    };
    ...
    let entries = match fs::read_dir(&worktrees_root) {
        ...
        Err(err) => {
            return (
                removed,
                vec![format!(
                    "could not read {} during prune: {err}",
                    display_path(&worktrees_root)
                )],
            );
        }
    };
    ...
    for entry in entries.flatten() {
        ...
        // If git still tracks any worktree under this root, skip it.
        // Some other run may legitimately own it.
        ...
        match fs::remove_dir_all(&path) {
            ...
            Err(err) => warnings.push(format!(
                "orphan worktree root {} could not be removed: {err}",
                display_path(&path)
            )),
        }
    }
...
fn all_managed_worktrees(workspace_root: &Path) -> Result<Vec<PathBuf>> {
...
fn copy_tree_recursive(
        ...
        .with_context(|| format!("failed to read {}", display_path(current_source)))?
    {
        ...
        if file_type.is_dir() {
            // Cargo build dirs contain CACHEDIR.TAG and explode under Windows MAX_PATH
            // once you nest a worktree underneath them. Skip the whole subtree.
            if is_cargo_build_dir(&source_path) {
                continue;
            }
