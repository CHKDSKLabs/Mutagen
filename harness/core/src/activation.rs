...
impl PreparedSliceReady {
    ...
    pub fn from_activation(activation: PreparedSliceActivation, queue_path: String) -> Self {
        let state_target = activation
            .slice
            .state_target()
            .expect("slice was validated before activation");
        ...
        Self {
            ...
            workspace_dirty: activation.workspace_dirty,
        }
    }
}
...
pub struct ActivateSliceOptions<'a> {
    ...
    pub host: HostKind,
    pub host_profile: HostExecutionProfile,
    pub queue_readiness: QueueReadinessSnapshot,
...
pub fn activate_slice(options: ActivateSliceOptions<'_>) -> Result<PreparedSliceActivation> {
    ...
    if let Some(reason) = slice.pending_human_check_reason() {
        bail!(
            "slice `{}` is waiting on a human check before execution: {}",
            slice.id,
            reason
        );
    }
    ...
    let mut active_state = ActiveSliceState::from_slice(
        &slice,
    ...
    active_state.set_queue_readiness(options.queue_readiness.clone());
    let workspace_dirty =
        summarize_dirty_paths(options.workspace_root, &active_state.allowed_write_globs);
