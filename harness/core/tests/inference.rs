...
use mutagen_core::adapter::{HostKind, resolved_host_profile};
...
fn ollama_and_lmstudio_profiles_are_advisory_and_serial() {
    ...
    for host in [HostKind::Ollama, HostKind::LmStudio] {
        ...
        assert_eq!(
            ...
            mutagen_core::adapter::ScopeEnforcementMode::Advisory,
            "{host:?}"
        );
        assert_eq!(
            profile.parallel_dispatch,
            mutagen_core::adapter::ParallelDispatchMode::SerialOnly,
            "{host:?}"
        );
