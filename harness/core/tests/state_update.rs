...
#[test]
fn apply_state_update_for_slice_uses_section_anchor_without_creating_pseudo_file() {
    let workspace = FixtureWorkspace::copy("basic_ready");
    let mut queue = workspace.read_json("slices/queue.json");
    queue["slices"][0]["context_to_update"] = serde_json::json!("project_state.md § RBAC");
...
"#,
    ...
    .expect("state update should apply to the canonical state file");

    assert_eq!(
        Path::new(&result.context_path)
            .file_name()
            .and_then(|name| name.to_str()),
        Some("project_state.md")
    );
    assert!(
        !workspace.root.join("project_state.md § RBAC").exists(),
        "section anchors are not filenames"
    );

    let project_state = workspace.read_text("project_state.md");
    assert!(project_state.contains("## RBAC"));
    assert!(project_state.contains("Existing notes."));
...
impl FixtureWorkspace {
    ...
    fn read_json(&self, relative_path: &str) -> serde_json::Value {
