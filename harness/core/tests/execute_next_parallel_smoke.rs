...
fn run_execute_next_drains_a_bounded_parallel_cohort() {
    if !bash_cargo_available() {
        eprintln!("skipping WSL cohort smoke: Linux cargo is not available in bash");
        return;
    }
...
fn run_bash(script: &str) {
    ...
    fs::write(&script_path, script).expect("bash smoke script should write");

    let output = Command::new("bash")
        .arg("-l")
        .arg(to_wsl_path(&script_path))
        .output()
...
fn run_execute_next_smoke(workspace: &FixtureWorkspace, env_exports: &[(&str, &str)]) -> Value {
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(|p| p.parent())
        .expect("mutagen-core crate should live under harness/ under repo root")
        .to_path_buf();
...
cargo run --quiet --manifest-path "{repo_root_wsl}/harness/cli/Cargo.toml" -- validate-queue --queue "{workspace_wsl}/slices/queue.json" > "{queue_validation_wsl}"
MUTAGEN_ROOT="{repo_root_wsl}/plugins/mutagen" CLAUDE_BIN="$fake_claude" cargo run --quiet --manifest-path "{repo_root_wsl}/harness/cli/Cargo.toml" -- run-execute-next \
