...
use mutagen_core::notifications::StopCondition;
use mutagen_core::queue::SliceQueue;
use mutagen_core::runtime::{PrepareNextOptions, PrepareNextResult, prepare_next};
use mutagen_core::validation::validate_queue;
...
fn prepare_next_claims_ready_slice_and_writes_state_and_evidence() {
    ...
    let evidence_bundle_path = match result {
        PrepareNextResult::Ready { prepared } => {
            ...
            assert_eq!(prepared.context_file, "project_state.md");
            assert_eq!(prepared.context_section, None);
            assert_eq!(
                prepared.write_set,
...
fn prepare_next_returns_stalled_when_dependencies_are_unmet() {
    ...
    match result {
        PrepareNextResult::Stalled {
            ...
            assert_eq!(blocked[0].pending_human_check, None);
...
#[test]
fn prepare_next_skips_unresolved_human_gated_slice() {
    let workspace = FixtureWorkspace::copy("basic_ready");
    ...
    match result {
        PrepareNextResult::Ready { prepared } => {
            assert_eq!(prepared.slice_id, "L2-orders-002");
        }
        other => panic!("expected ready result for the ungated slice, got {other:?}"),
    }
}

#[test]
fn prepare_next_reports_unresolved_human_check_as_blocked() {
    let workspace = FixtureWorkspace::copy("basic_ready");
    let mut queue = workspace.read_json("slices/queue.json");
    queue["slices"][0]["human_check_needed"] = json!({
        "required": true,
    ...
    let result = prepare_next(workspace.prepare_next_options(false))
        .expect("prepare-next should report stalled state");

    match result {
        PrepareNextResult::Stalled { blocked, .. } => {
            assert_eq!(blocked.len(), 2);
            assert_eq!(blocked[0].id, "L1-orders-001");
            assert_eq!(
                blocked[0].pending_human_check.as_deref(),
                Some("Project owner needs to pick the API shape.")
            );
        }
        other => panic!("expected stalled result, got {other:?}"),
    }
    ...
    match result {
        PrepareNextResult::QueueClear {
            ...
            assert_eq!(
                ...
                mutagen_core::notifications::NotificationKind::QueueClear
            );
        }
...
#[test]
fn prepare_next_refuses_missing_queue_validation() {
    let workspace = FixtureWorkspace::copy_without_validation("basic_ready");

    let error = prepare_next(workspace.prepare_next_options(false))
        .expect_err("prepare-next should require queue validation");

    assert!(
        error
            .to_string()
            .contains("Queue validation report is missing")
    );
}

#[test]
fn prepare_next_refuses_stale_queue_validation() {
    let workspace = FixtureWorkspace::copy("basic_ready");
    let mut queue = workspace.read_json("slices/queue.json");
    queue["slices"][0]["objective"] = json!("Changed after validation.");
    workspace.write_json("slices/queue.json", &queue);

    let error = prepare_next(workspace.prepare_next_options(false))
        .expect_err("prepare-next should require fresh queue validation");

    assert!(
        error
            .to_string()
            .contains("Queue validation report is stale")
    );
}
...
#[test]
fn validate_queue_rejects_parenthetical_state_target() {
    let workspace = FixtureWorkspace::copy("basic_ready");
    let mut queue = workspace.read_json("slices/queue.json");
    queue["slices"][0]["context_to_update"] = json!("project_state.md (RBAC section)");
    ...
    assert!(
        report.issues.iter().any(|issue| {
            issue.code == "slice_contract"
                && issue
                    .message
                    .contains("combines a file and section label as a path")
        }),
        "expected parenthetical target validation error, got {:?}",
        report.issues
    );
}
...
impl FixtureWorkspace {
    fn copy(name: &str) -> Self {
        let workspace = Self::copy_without_validation(name);
        support::write_queue_validation(&workspace.root);
        workspace
    }

    fn copy_without_validation(name: &str) -> Self {
