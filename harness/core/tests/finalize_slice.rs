...
use mutagen_core::notifications::NotificationKind;
...
"#,
    ...
    // the fixture's write_set is ["src/orders/**", "tests/orders/**"], so we
    // have to leave at least one real file under each prefix.
    workspace.write_text("src/orders/aggregate.rs", "// placeholder\n");
    workspace.write_text("tests/orders/aggregate_test.rs", "// placeholder\n");
...
fn finalize_slice_refuses_when_state_update_block_is_missing_slice_marker() {
    let workspace = FixtureWorkspace::copy("basic_ready");
    workspace.prepare_claimed_slice();
    // Past the L1-Harness-002 artifact gate, the test has to seed the
    // declared write_set on disk so finalize gets far enough to evaluate
...
"#,
    ...
    assert!(
        error
            .to_string()
            .contains("State Update block must contain a slice marker"),
