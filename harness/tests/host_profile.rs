use mutagen_harness::adapter::{ParallelDispatchMode, ScopeEnforcementMode, resolved_host_profile};
...
fn codex_profile_degrades_parallel_dispatch_when_worktrees_are_unavailable() {
    ...
    let profile =
        resolved_host_profile(mutagen_harness::adapter::HostKind::Codex, &workflow_config);
...
fn claude_profile_preserves_bounded_parallel_dispatch_when_requested() {
    ...
    let profile =
        resolved_host_profile(mutagen_harness::adapter::HostKind::Claude, &workflow_config);
