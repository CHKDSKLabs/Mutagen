...
- **Plugin author identity aligned.** `.codex-plugin/plugin.json#author` was `CHKDSK Labs`; aligned to `ObtuseAglet` to match `.claude-plugin/plugin.json` and `.claude-plugin/marketplace.json`. The Codex `interface.developerName` field stays `CHKDSK Labs` since that's the display name slot.
