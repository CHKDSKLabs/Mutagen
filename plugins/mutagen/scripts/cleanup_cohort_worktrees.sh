...
if [[ ! -f "$MANIFEST_PATH" ]]; then
  emit_error "cleanup_cohort_worktrees_unavailable" "mutagen harness manifest not found"
fi

CARGO_BIN="$(resolve_cargo)" || emit_error "cleanup_cohort_worktrees_unavailable" "cargo not found on PATH"
JQ_BIN="$(resolve_jq)" || emit_error "cleanup_cohort_worktrees_unavailable" "jq not found on PATH"

set +e
OUTPUT="$(
  "$CARGO_BIN" run --quiet --manifest-path "$MANIFEST_PATH" -- cleanup-cohort-worktrees "${HARNESS_ARGS[@]}" 2>&1
)"
STATUS=$?
set -e
