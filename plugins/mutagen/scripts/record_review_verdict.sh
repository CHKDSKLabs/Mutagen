...
if [[ ! -f "$MANIFEST_PATH" ]]; then
  emit_error "record_review_verdict_unavailable" "mutagen harness manifest not found"
fi

CARGO_BIN="$(resolve_cargo)" || emit_error "record_review_verdict_unavailable" "cargo not found on PATH"
JQ_BIN="$(resolve_jq)" || emit_error "record_review_verdict_unavailable" "jq not found on PATH"

set +e
OUTPUT="$(
  "$CARGO_BIN" run --quiet --manifest-path "$MANIFEST_PATH" -- record-review-verdict "${HARNESS_ARGS[@]}" 2>&1
)"
STATUS=$?
set -e
