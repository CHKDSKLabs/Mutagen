...
if [[ ! -f "$MANIFEST_PATH" ]]; then
  emit_error "reconcile_cohort_member_unavailable" "mutagen harness manifest not found"
fi

CARGO_BIN="$(resolve_cargo)" || emit_error "reconcile_cohort_member_unavailable" "cargo not found on PATH"
JQ_BIN="$(resolve_jq)" || emit_error "reconcile_cohort_member_unavailable" "jq not found on PATH"

set +e
OUTPUT="$(
  "$CARGO_BIN" run --quiet --manifest-path "$MANIFEST_PATH" -- reconcile-cohort-member "${HARNESS_ARGS[@]}" 2>&1
)"
STATUS=$?
set -e
