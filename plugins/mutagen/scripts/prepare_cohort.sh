#!/usr/bin/env bash
# Select and prepare a bounded parallel cohort through the Rust harness.
#
# Usage:
#   prepare_cohort.sh [--workspace-root PATH] [--queue PATH]
#                     [--queue-validation PATH] [--workflow-config PATH]
#                     [--host HOST] [--dry-run]
#
# Exit codes:
#   0 = prepare-cohort completed
...
WORKFLOW_CONFIG_PATH="$(workspace_relative_path "$WORKSPACE_ROOT" "$WORKFLOW_CONFIG_PATH")"
...
if [[ $PREPARE_STATUS -ne 0 ]]; then
  printf '%s' "$PREPARE_OUTPUT" | "$JQ_BIN" -Rs \
    ...
    '{
      ok: false,
      reason: "prepare_cohort_runtime_failure",
      workspace_root: $workspace_root,
      queue: $queue,
      workflow_config: $workflow_config,
      host: $host,
...
if ! printf '%s' "$PREPARE_OUTPUT" | "$JQ_BIN" empty >/dev/null 2>&1; then
  printf '%s' "$PREPARE_OUTPUT" | "$JQ_BIN" -Rs \
    ...
    '{
      ...
      workflow_config: $workflow_config,
      host: $host,
      message: ("prepare-cohort returned non-JSON output: " + .)
    }'
  exit 1
fi
